...
impl <I,O>Value<I,O> {
    pub fn input(&self) -> &I {
        if let Value::Input(i) = self { i } else { panic!("Not a input") }
    }

    pub fn output(&self) -> &O {
        if let Value::Output(o) = self { o } else { panic!("Not a output") }
    }
}
...
pub trait Model : Clone + Send + 'static {
    ...
    fn partition_event(&self, history: Vec<Event<Value<Self::Input, Self::Output>>>) -> Vec<Vec<Event<Value<Self::Input, Self::Output>>>> {
        vec![history]
    }
